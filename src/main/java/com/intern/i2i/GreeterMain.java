...
public class GreeterMain extends AbstractBehavior<GreeterMain.SayHello> {
	...
	public static class AnswerToHello{
		public final String name;
		public AnswerToHello(String name) {
			this.name = name;
		}
	}
	...
	private GreeterMain(ActorContext<SayHello> context) {
		super(context);
		// #create-actors
		greeter = context.spawn(Greeter.create(), "ActorA");
		// #create-actors
	}
	...
	private Behavior<SayHello> onSayHello(SayHello command) {
		// #create-actors
		ActorRef<Greeter.Greeted> replyTo = getContext().spawn(GreeterBot.create(1), command.name);
